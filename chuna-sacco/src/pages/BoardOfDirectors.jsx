...
const SupervisoryCommittee = () => {
  ...
  useEffect(() => {
    const fetchSupervisoryMembers = async () => {
      try {
        setLoading(true);
        console.log('Fetching from:', `${API_BASE_URL}/board`);
        ...
        if (!contentType || !contentType.includes('application/json')) {
          ...
          throw new Error('Server returned non-JSON response. Check backend endpoint.');
        }
        ...
        if (!data.supervisory) {
          console.error('No supervisory field in response:', data);
          throw new Error('Invalid response format - missing supervisory field');
        }
        ...
        const mappedMembers = data.supervisory.map(member => ({
          ...
          image: member.photo_url || 'https://via.placeholder.com/300x400?text=No+Image',
        ...
        console.error('Error fetching supervisory members:', err);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">No Committee Members</h2>
          <p className="text-gray-600">Supervisory committee members will appear here once added.</p>
        </div>
      </div>
    );
  }
